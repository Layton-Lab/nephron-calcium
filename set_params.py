...
def read_params(cell,filename,j):
    ...
    # parameter files specify superficial segmental lengths; for some segments, lengths are different for juxtamedullary nephrons, so we overwrite them here...
            ...
            # Specific transporters:
                ...
                # Ca2+ specific inhibitions:       
                if cell.inhib == 'TRPV5-99':
                    if newTransp.type == 'TRPV5':
                        newTransp.act = value/(href*Cref)*(1-0.999)
                if cell.inhib == 'NCX1-99':
                        if newTransp.type == 'NCX1':
                            newTransp.act = value/(href*Cref)*(1-0.999)
                ...
                if (cell.segment == 'DCT') and (j<2.0/3.0*cell.total) and (newTransp.type == 'TRPV5'):
                    newTransp.act = 0
					
                if (cell.segment == 'DCT') and (j<2.0/3.0*cell.total) and (newTransp.type == 'NCX1'):
                    newTransp.act = 0
					
                if (cell.segment == 'DCT') and (j<2.0/3.0*cell.total) and (newTransp.type == 'CaATPase'):
                    newTransp.act = 0
