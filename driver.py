    ...
    for i in range(N-1):
        ...
        # Trancellular and paracellular Mg2+ transport is not considered for CNT, CCD, and OMCD
            ...
            if cell[0].segment == 'CNT':
                ...
                for j in range(NC-1):
                    x[5*NS+j-2]=cell[i].vol[j]
                    x[5*NS+5+j-2]=cell[i].ep[j]
                x[5*NS+8]=cell[i].pres[0]
                ...
                for j in range(NC-1):
                    x[5*NS+j-3]=cell[i].vol[j]
                    x[5*NS+5+j-3]=cell[i].ep[j]
                x[5*NS+7]=cell[i].pres[0]
        else:
        ...
        if cell[0].segment == 'PT' or cell[0].segment == 'S3' or cell[0].segment =='SDL' or cell[0].segment == 'LDL' or cell[0].segment == 'LAL' or cell[0].segment == 'mTAL' or cell[0].segment == 'cTAL' or cell[0].segment == 'MD' or cell[0].segment == 'DCT' or cell[0].segment == 'IMCD':
            ...
            if cell[0].segment in ['PT','S3', 'SDL']:
                ...
                if i < 2.0 / 3.0 * cell[0].total:
                    cell[i+1].conc[:, 0] = sol[0:NS]
                    cell[i+1].conc[:15, 1] = sol[NS:2*NS-1]
                ...
                cell[i+1].conc[:,0] = sol[0:NS]
                cell[i+1].conc[:15,1] = sol[NS:NS*2-1]
                cell[i+1].conc[:,4] = sol[NS*2-1:NS*3-1]
            ...
            if cell[0].segment == 'CNT':
                ...
                for j in range(NC-1):
                    cell[i+1].vol[j] = sol[5*NS+j-2]
                    cell[i+1].ep[j] = sol[5*NS+5+j-2]
        
                cell[i+1].pres[0] = sol[5*NS+8]
            else:
                for j in range(NS):
                    if j != 15:
                        cell[i+1].conc[j,0] = sol[5*j]
                    ...
                    for j in range(NC-1):
                        cell[i+1].vol[j] = sol[5*NS+j-3]
                        cell[i+1].ep[j] = sol[5*NS+5+j-3]
        
                    cell[i+1].pres[0] = sol[5*NS+7]  
...
#================================OUTPUT IN TO FILE================================        
    ...
    solute = ['Na','K','Cl','HCO3','H2CO3','CO2','HPO4','H2PO4','urea','NH3','NH4','H','HCO2','H2CO2','glu', 'Ca']
