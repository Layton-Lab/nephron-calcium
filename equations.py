...
#---------------------------------------------------------------------    
       # PT and S3 are the only compliant segments.
        ...
        if cell1.segment =='CNT':
            ...
            for j in range(NC-1):
                cell1.vol[j]=x[5*NS+j-2]
                cell1.ep[j]=x[5*NS+5+j-2]
            cell1.pres[0]= x[5*NS+8]
        else:
            for j in range(NS):
                if j != 15:
                    cell1.conc[j,0] = x[5*j]
            ...
            for j in range(NC-1):
                cell1.vol[j]=x[5*NS+j-3]
                cell1.ep[j]=x[5*NS+5+j-3]
            cell1.pres[0]=x[5*NS+7]
    ...
    #---------------------------------------------------------------------
        ...
        '''
        if cell1.segment == 'CNT':
            fvec = np.delete(fvec, [77,78])
        else:
            fvec = np.delete(fvec,[76,77,78])
